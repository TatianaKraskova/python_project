...
def recommended_locators(page: Page):
    ...
    # Additional interactions (from the code generator)
    page.get_by_label("First name").click()
    page.get_by_label("First name").fill("Maria")
    expect(page.get_by_label("First name")).to_be_visible()

    # Verify "Last name" is empty and then fill it
    expect(page.get_by_label("Last name")).to_be_empty()
